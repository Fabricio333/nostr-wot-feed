...
export function NoteThread() {
  ...
  useEffect(() => {
    ...
    pool.querySync(Relay.getUrls(), { ids: [id] }).then((events) => {
      if (events.length > 0) {
  ...
  async function fetchParentChain(note: Note) {
    ...
    while (currentReplyTo && depth < 10) {
      ...
      // Check ParentNotes cache
      const parentData = ParentNotes.get(currentReplyTo);
      if (parentData) {
        // We have partial data, try to fetch full event
        try {
          const events = await pool.querySync(Relay.getUrls(), { ids: [currentReplyTo] });
          if (events.length > 0) {
  ...
  async function fetchReplies(noteId: string) {
    ...
    try {
      const events: NostrEvent[] = await pool.querySync(
        Relay.getUrls(),
        { kinds: [1], '#e': [noteId], limit: 50 } as any
      );
  ...
  return (
    <div className="bg-black min-h-screen text-white pb-24 md:pb-0">
      ...
      <header className="sticky top-0 z-10 bg-black/80 backdrop-blur-md border-b border-zinc-800 p-4 flex items-center gap-3">
        <Link to="/" className="p-1 -ml-1 hover:bg-zinc-800 rounded-full transition-colors">
          <ArrowLeft size={20} />
        </Link>
      ...
      {!loading && !targetNote && (
        <div className="text-center py-16 text-zinc-500">
          ...
          <Link to="/" className="text-purple-400 text-sm hover:text-purple-300 mt-4 inline-block">
            Back to feed
          </Link>
        </div>
      )}
