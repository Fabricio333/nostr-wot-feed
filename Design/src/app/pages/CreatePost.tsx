...
import { X, Image, MapPin, Smile, Hash, Camera } from 'lucide-react';
...
export function CreatePost() {
  ...
  const handlePost = () => {
    ...
    console.log('Posting:', { type: activeTab, text, media });
    navigate('/');
  };
    ...
    <div className="flex flex-col h-screen bg-black text-white relative">
      <div className="flex items-center justify-between p-4 border-b border-zinc-800">
        ...
        <button 
          onClick={handlePost}
          disabled={!text && !media}
          className="font-bold text-blue-500 disabled:opacity-50 hover:text-blue-400 transition-colors"
        >
          Share
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex gap-3">
          <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-800 flex-shrink-0">
            <img src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&h=100&fit=crop" alt="Current User" className="w-full h-full object-cover" />
          </div>
