...
export function TrendingSidebar() {
  ...
  const handleHashtagClick = (tag: string) => {
    router.push(`/explore?q=%23${encodeURIComponent(tag)}`);
  };
...
function TrendingPostCard({ post }: { post: TrendingPost }) {
  ...
  return (
    <div className="px-2 py-1.5 rounded-lg hover:bg-zinc-800 transition-colors group cursor-pointer" onClick={() => router.push(`/note/${post.id}`)}>
      {/* Author row */}
