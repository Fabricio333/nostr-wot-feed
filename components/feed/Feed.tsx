...
import { RelayStats } from '@/lib/nostr/relayStats';
import { Verifier } from '@/lib/nostr/verifier';
...
export function Feed() {
  ...
  const { isConnected: wotExtDetected } = useExtension();
  const { initFeed, subscribeFollowing, connectedCount, totalRelays } = useRelayPool();
      ...
      <header className="sticky top-0 z-10 bg-black/80 backdrop-blur-md border-b border-zinc-800">
        <div className="px-4 pt-3 pb-0 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 text-xs text-zinc-500">
            ...
            <StatusPill status={relayStatus} connected={connectedCount} total={totalRelays} />
          </div>
        </div>
