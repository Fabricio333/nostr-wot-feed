...
export function Settings() {
  ...
  const { isConnected: hasExtension } = useExtension();
  const { updateTick } = useProfileStore();
  const {
    connectedCount,
    relayStatuses,
    addRelay: relayPoolAddRelay,
    removeRelay: relayPoolRemoveRelay,
    reconnect,
    fetchUserRelays,
    refreshStatuses,
  } = useRelayPool();
  ...
  const handleAddRelay = () => {
    ...
    relayPoolAddRelay(url);
    setNewRelay('');
  };

  const handleRemoveRelay = (url: string) => {
    settings.removeRelay(url);
    relayPoolRemoveRelay(url);
  };
  ...
  return (
    <div className="bg-black min-h-screen text-white pb-20 md:pb-0">
      ...
      <div className="max-w-xl mx-auto p-4 space-y-6">
        ...
        {/* Relays */}
        <Section title={`Relays (${connectedCount}/${settings.relays.length} connected)`}>
          <div className="space-y-2">
            {settings.relays.map((url) => {
              const isConnected = relayStatuses.get(url);
              const dotColor =
